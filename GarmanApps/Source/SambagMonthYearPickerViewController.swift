...
public class SambagMonthYearPickerViewController: UIViewController {
    ...
    public override func loadView() {
        ...
        if subjects[0] == "Very Low"{
            titleLabel = UILabel()
            titleLabel.text = "Set urgency"
            titleLabel.textColor = attrib.titleTextColor
            titleLabel.font = attrib.titleFont
        }else{
            titleLabel = UILabel()
            titleLabel.text = "Set subject"
            titleLabel.textColor = attrib.titleTextColor
            titleLabel.font = attrib.titleFont
        }
        ...
        if subjects[0] == "Very Low"{
            yearWheel.selectedIndexPath.row = 2
        }else{
            yearWheel.selectedIndexPath.row = 0
        }
    ...
    public override func viewDidLayoutSubviews() {
        ...
        let wheelWidth: CGFloat = view.frame.width - 100
        let wheelSpacing: CGFloat = 30
        let totalWidth: CGFloat = wheelWidth  + wheelSpacing
        
        rect.origin.y = rect.maxY + 20
        rect.size.width = wheelWidth
        rect.size.height = 210
        rect.origin.x = rect.minX + 30
...
extension SambagMonthYearPickerViewController: SambagMonthYearPickerViewControllerInteraction {
    ...
    func didTapSet() {
        ...
        var selectedThing = subjects[yearWheel.selectedIndexPath.row]
        print(selectedThing)
        
        var result = SambagMonthYearPickerResult()
        result.tot = selectedThing
